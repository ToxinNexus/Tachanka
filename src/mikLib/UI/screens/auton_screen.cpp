...
controller_btn::controller_btn(bool is_btn, std::string unpressed_state, std::string pressed_state, std::function<void()> callback) : is_btn(is_btn), unpressed_state(unpressed_state), pressed_state(pressed_state), callback(callback)
{
    state_label = unpressed_state;
};

controller_btn::controller_btn(bool really_stupid_exception_that_shouldnt_exist_but_i_dont_want_to_make_another_class, std::function<void()> callback) : callback(callback), exception(really_stupid_exception_that_shouldnt_exist_but_i_dont_want_to_make_another_class)
{
...
void controller_btn::unpush()
{
    state = true;
    push();
}
void controller_btn::enable_full_override(std::string lbl)
{
    state_label = lbl;
    full_override = true;
}
...
void UI_auton_screen::UI_crt_auton_scr()
{
    ...
    red_blue_tgl = UI_crt_tgl(UI_crt_grp({UI_crt_cir(1.19, .6, .25, .25, auton_toggle_blue_bg_color, UI_distance_units::inches),
    ...
    red_blue_tgl->set_callback([this]()
                               {  UI_select_auton(autons::RED_BLUE); flip_toggle_controller({0, 0}, red_blue); save_auton_SD(); }, false);

    rings_goal_tgl = UI_crt_tgl(UI_crt_grp({UI_crt_cir(1.19, 1.1, .25, .25, auton_toggle_left_bg_color, UI_distance_units::inches),
    ...
    rings_goal_tgl->set_callback([this]()
                                 { UI_select_auton(autons::RINGS_GOAL); flip_toggle_controller({0, 1}, rings_goal); save_auton_SD(); }, false);

    quals_elims_tgl = UI_crt_tgl(UI_crt_grp({UI_crt_cir(1.19, 1.6, .25, .25, auton_toggle_quals_bg_color, UI_distance_units::inches),
    ...
    quals_elims_tgl->set_callback([this]()
                                  { UI_select_auton(autons::QUALS_ELIMS); flip_toggle_controller({0, 2}, quals_elims); save_auton_SD(); }, false);

    off_sawp_tgl = UI_crt_tgl(UI_crt_grp({UI_crt_cir(1.19, 2.1, .25, .25, auton_toggle_off_bg_color, UI_distance_units::inches),
    ...
    off_sawp_tgl->set_callback([this]()
                               { UI_select_auton(autons::OFF_SAWP); flip_toggle_controller({1, 0}, off_sawp); save_auton_SD(); }, false);
    ...
    show_alignment_btn->set_callback([this]()
                                     { queue_autons(true, false); });

    auto heading_btn = UI_crt_btn(UI_crt_rec(268, 50, 170, 76, auton_data_bg_color, auton_data_outline_color, 2, UI_distance_units::pixels), []()
                                  { calibrate_inertial(); });
    ...
    auto heading_lbl = UI_crt_lbl("", []()
                                  { return chassis.get_absolute_heading(); }, 350 - 10, 75, auton_text_color, auton_bg_color, UI_distance_units::pixels);
    auto x_lbl = UI_crt_lbl("", []()
                            { return chassis.get_X_position(); }, 350 - 10, 95, auton_text_color, auton_bg_color, UI_distance_units::pixels);
    auto y_lbl = UI_crt_lbl("", []()
                            { return chassis.get_Y_position(); }, 350 - 10, 115, auton_text_color, auton_bg_color, UI_distance_units::pixels);
    ...
    auto prev_auto_var_btn = UI_crt_btn(UI_crt_grp({
                                        ...
                                        [this]()
                                        { next_var(); update_var_display(); flip_toggle_controller({1, 2}); save_auton_SD(var_num); });
...
void UI_auton_screen::queue_autons(bool calibrating, bool set_description)
{
    ...
    if (off_skills)
    {
        output = skills(calibrating, var, set_description);
        return;
    }
...
void UI_auton_screen::controller_default_scr()
{
    controller_scr_overlay = vex::task([]()
        ...
        while(1) {
            ...
            Controller.Screen.setCursor(2, 1);
            if (middle == false) {
                Controller.Screen.print("High");
            } else if (middle == true) {
                Controller.Screen.print("Middle");
            }
            Controller.Screen.setCursor(3, 1);
            if (active == false) {
                Controller.Screen.print("Neutral");
            } else if (active == true) {
                Controller.Screen.print("Active");
            }
            vex::task::sleep(1000);
        }
        return 0; });
...
void UI_auton_screen::enable_controller_overlay()
{
    ...
    control_panel = {
        {{controller_btn(false, "[Blue]", "[Red]", [this]()
                         { UI_select_auton(autons::RED_BLUE); flip_toggle(red_blue_tgl, red_blue); save_auton_SD(); })},
         {controller_btn(false, "[Left]", "[Right]", [this]()
                         { UI_select_auton(autons::RINGS_GOAL); flip_toggle(rings_goal_tgl, rings_goal); save_auton_SD(); })},
         {controller_btn(false, "[Mid]", "[No Mid]", [this]()
                         { UI_select_auton(autons::QUALS_ELIMS); flip_toggle(quals_elims_tgl, quals_elims); save_auton_SD(); })}},

        {{controller_btn(false, "[Off]", "[Sawp]", [this]()
                         { UI_select_auton(autons::OFF_SAWP); flip_toggle(off_sawp_tgl, off_sawp); save_auton_SD(); })},
         {controller_btn(false, "[Off]", "[Skills]", [this]()
                         { UI_select_auton(autons::OFF_SKILLS); flip_toggle(config_scr->macro_10_bg_tgl, off_skills); save_auton_SD(); })},
         {controller_btn(true, [this]()
                         { next_var(); update_var_display(); flip_toggle_controller({1, 2}); save_auton_SD(var_num); })}},

        {{controller_btn(true, "[Desc]", "[###]", [this]()
                         { controller_description_scr(); })},
         {controller_btn(true, "[Calib]", "[###]", [this]()
                         { controller_calibrate_scr(); queue_autons(true, false); })},
         {controller_btn(true, "[Run]", "[##]", []()
                         { auton_scr->start_auton_test(); })}},

        {{controller_btn(false, "[Off]", "[Cap]", [this]()
                         { enable_time_limit(); flip_toggle(config_scr->macro_4_bg_tgl, time_limit); save_auton_SD(); })},
         {controller_btn(true, "[RCal]", "[...]", []()
                         { calibrate_inertial(); })},
         {controller_btn(false, "[Off]", "[Odom]", [this]()
                         { enable_odom_display(); save_auton_SD(); })}}};
