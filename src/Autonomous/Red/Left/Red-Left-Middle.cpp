...
std::string red_left_middle(bool calibrate, mik::auto_variation var, bool get_name)
{
    ...
    active = false;
    assembly.scraper.toggle();
    chassis.right_swing_to_point(26, 9);
    chassis.drive_to_point(26, 9);
    chassis.turn_to_point(40.5, -7, {.angle_offset = 180});
    intakecon = 3;
    middle = true;
    active = true;
    chassis.drive_to_point(40.5, -7);
    intakecon = 1;
    task::sleep(1000);
    middle = false;
    active = false;
    chassis.drive_to_point(4.25, 40.25);
    chassis.turn_to_point(19.25, 40.25, {.angle_offset = 180});
    assembly.wing.toggle();
    task::sleep(250);
    assembly.wing.toggle();
    chassis.drive_distance(-20, {.timeout = 850});
