...
std::string red_left_no_middle(bool calibrate, mik::auto_variation var, bool get_name) {
    ...
    if (get_name) {
        if (var == one) {return "Red Left No Middle (Wing)";}
        if (var == two) {return "Red Left No Middle (Hood)";}
        if (var == three) {return "Red Left Goal Rush";}
    }
    if (calibrate) {
        odom_constants();
    ...
    while (chassis.is_in_motion()) {task::sleep(25);}
