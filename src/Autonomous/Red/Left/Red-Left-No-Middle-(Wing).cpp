...
std::string red_left_no_middle_wing(bool calibrate, mik::auto_variation var, bool get_name)
{
    if (var == one)
    {
    }
    if (var == two)
    {
        return red_left_no_middle_hood(calibrate, var, get_name);
    }
    if (var == three)
    {
    ...
    if (get_name)
    {
        return "Red Left No Middle (Wing)";
    }
    if (calibrate)
    {
        odom_constants();
